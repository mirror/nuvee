...
BOOL CALLBACK DInput_EnumCallback( const DIDEVICEINSTANCE* instance, VOID* context )
{
	if( FAILED( dinput->CreateDevice( instance->guidInstance, &di_mouse[ di_mouse_count ], NULL )) )
		return DIENUM_CONTINUE;

	if( FAILED( di_mouse[ di_mouse_count ]->SetDataFormat( &c_dfDIMouse2 )) )
		return DIENUM_CONTINUE;
...
BOOL CALLBACK DInput_joystick_enumAxesCallback( const DIDEVICEOBJECTINSTANCE* instance, VOID* context )
{
	...
	// Mouse range = 8-bit values (limit range)
	propRange.lMin              = -64;
	propRange.lMax              = +64;
...
void Init_DInput( HINSTANCE hInstance, HWND hWnd )
{
	...
	if( hidmouse_api == HIDMOUSE_API_DIRECTINPUT )
	{
		...
		dinput->EnumDevices( DI8DEVCLASS_POINTER, DInput_EnumCallback, NULL, DIEDFL_ATTACHEDONLY );
	}
	...
	if( FAILED( di_keyboard->SetCooperativeLevel( hWnd_app_key, DISCL_NONEXCLUSIVE | DISCL_FOREGROUND )) )
		return;
}
...
void Poll_Mouse_DInput()
{
	...
	for( int lcv = 0; lcv < di_mouse_count; lcv++ )
	{
		...
		hr = di_mouse[ lcv ]->GetDeviceState( sizeof(DIMOUSESTATE2), (LPVOID)&di_mousestate );
		if( FAILED(hr) ) continue;
